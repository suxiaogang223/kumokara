# bash shell integration for Kumokara
...
if [[ -z "$KUMOKARA_DISABLE_INTEGRATION" ]]; then
    ...
    trap '_kumokara_preexec' DEBUG

    # Use PROMPT_COMMAND for precmd-like behavior
    PROMPT_COMMAND="_kumokara_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}"

    # Add prompt marker
    PS1="\[$(_kumokara_prompt_marker)\]$PS1"
fi
